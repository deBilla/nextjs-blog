---
title: "I Hired a Platform Engineer Who Can’t Break Anything"
date: "2026-07-29"
preview: "Onboarding an AI agent onto my cloud with 15 tools, viewer-only credentials, and no path to production"
description: "Building a read-only platform agent that does the on-call log and metric hunting for you, with no ability to change anything."
tags: ["react", "ai", "devops"]
mediumUrl: "https://medium.com/@billacode/i-hired-a-platform-engineer-who-cant-break-anything-fb87d6778f69"
---
...
The strange thing is that none of that is hard work. It’s _joining_ work — correlate this log spike with that deploy, with that memory metric, with that error group’s first-seen timestamp. It’s exactly the kind of work a language model is good at, and exactly the kind of work it can’t do, because it can’t see any of it.
...
![This is a walkthrough of the implementation, including the decisions I’d defend and the bugs that were only obvious in hindsight](./images/i-hired-a-platform-engineer-who-cant-break-anything/1.png)
...
def query_logs(filter: str = "", freshness: str = "1h", limit: int = 50) -> dict:
    """Query Cloud Logging with an advanced-filter expression.Args:
...
One sign convention to watch: the Recommender API expresses savings as a _negative_ cost delta. A $200/month saving arrives as `-200`. Sorting ascending puts the biggest wins first, and the total gets flipped once at the boundary:
...
That last one is where read-only stops feeling like a limitation. The agent produces a _plan_. I decide whether to run it. The division of labor — model does the correlating, human does the mutating — turns out to be the right one far more often than it’s a constraint.
...
- **Tool descriptions are the interface.** Your docstrings are the API contract with a model that has no ability to read your source. Write examples in them. Iterate on them the way you’d iterate on a UI.
- **Payload shaping is a correctness concern, not an optimization.** Return the five fields that matter. A model given everything performs worse than a model given the right thing, and it costs more.
- **Capability boundaries belong in infrastructure, not code.** “My server never calls a write API” is a comment. “This credential has viewer roles” is a guarantee. Build the second one.
